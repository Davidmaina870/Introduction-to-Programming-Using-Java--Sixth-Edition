...
%XALAN_COMMAND% -xsl convert-tex.xsl -in javanotes6-tex.xml -out tex/javanotes6.tex
...
%LATEX_COMMAND% javanotes6.tex
%LATEX_COMMAND% javanotes6.tex
%LATEX_COMMAND% javanotes6.tex

if not exist javanotes6.dvi  (
   echo.
...
if not "%DVIPDF_COMMAND%" == """" (
   "%DVIPDF_COMMAND%" javanotes6.dvi
)
else (
   "%DVIPS_COMMAND%" -Ppdf javanotes6.dvi -o 
   "%PS2PDF_COMMAND%" javanotes6.ps
)


if not exist javanotes6.pdf (
   echo.
...
move javanotes6.pdf "%BUILD_OUTPUT_DIR%"
...
if  not "%KEEP_LATEX%" == "no" (
   if exist "%BUILD_OUTPUT_DIR%"\latex-source rmdir/S/Q "%BUILD_OUTPUT_DIR%"\latex-source
   move tex "%BUILD_OUTPUT_DIR%"\latex-source
