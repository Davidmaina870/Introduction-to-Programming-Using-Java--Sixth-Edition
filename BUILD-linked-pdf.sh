...
if ! $XALAN_COMMAND -xsl convert-tex-linked.xsl -in javanotes6-tex.xml -out tex/javanotes6-linked.tex ; then
   echo
...
if [ ! -e "javanotes6-linked.dvi" ] ; then
   echo
   echo "An error occurred while trying to run latex on javanotes6-linked.tex."
...
if [ ! -e "javanotes6-linked.pdf" ] ; then
   echo
   echo "An error occurred while trying to run dvipdf on javanotes6-linked.dvi."
   echo "Exiting from BUILD-PDF.sh; latex files are in $JAVA_SOURCE_DIR/tex"
   echo
   cd ..
   exit 1
...
if ! mv javanotes6-linked.pdf $BUILD_OUTPUT_DIR ; then
...
echo "javanotes6-linked.pdf created in $BUILD_OUTPUT_DIR"
